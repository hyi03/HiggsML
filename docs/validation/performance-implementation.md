...
实施日期：2026-09-11。设计与对照版本：[重构方案](performance-design.md)，`66784d3af0a4b73c6001acc94404da3540040866`。
...
研究 CLI 各阶段可传入 `--resources config/examples/resources.json`。省略时使用同样默认值：
...
合成测量原始记录见 [performance-synthetic-results.json](performance-synthetic-results.json)。记录包含三次耗时、范围、RSS 样本、软件版本和实施文件摘要，科学数值比较预设 `rtol=atol=1e-12`，离散状态与选择要求相同。200 epoch 基准核对完整 history、checkpoint 和选择轮次；ROOT 吞吐样例不包含完整选择/特征重建。
