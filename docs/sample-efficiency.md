...
The main batch independently retrains the same 15 subsets with explicit `m4l` off and reports paired global AUC, registered mass-slice AUC with local support, and T1 W68 effects. The later off-only attribution layer adds a deterministic `M0off` empty set and computes a separate off-family Shapley decomposition without retraining those models. That exploratory off-only result does not participate in the mass-conditioned compact-candidate selection or change this sample-efficiency protocol. Also record efficiencies, score distributions, parameter counts, epochs, and failures; negative effects and unestimable regions are retained.
