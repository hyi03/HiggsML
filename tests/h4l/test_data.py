...
from higgsml.data import (assign_roles, audit_g0, grouped_statistics, load_research_data,
                               role_for_group, write_research_data)
from higgsml.errors import ResearchError
from higgsml.protocol import load_protocol
from higgsml.modeling.representations import ENGINEERED19, ordered_group_subsets, representation_features
...
def test_seed42_batch_config_registers_all_fifteen_combinations_once():
    config = json.loads((PROJECT_ROOT / "config/protocols/feature_combinations_seed42.json").read_text(encoding="utf-8"))
...
def test_mc_source_rejected_before_uproot_open(tmp_path, monkeypatch):
    from higgsml.data import export_research_data
    import uproot
...
def test_root_export_selects_protocol_population_before_payload(
    ...
    import uproot
    import higgsml.dataset_binding as binding
    ...
    protocol = load_protocol(PROJECT_ROOT / "config" / "protocols" / protocol_name)
    metrics = {}
