...
from higgsml.inference import marginal_workflow as w
...
def _fixture(tmp_path,monkeypatch,negative=False):
    ...
    registered=_run(root,'register',protocol,{'registration.json':{}},'attribution-v3-register')
    nominal=_run(root,'nominal',protocol,{},'attribution-v3-nominal')
...
def test_poisoned_assessment_j0_j1_acyclic_spec_and_freeze(tmp_path,monkeypatch):
    ...
    for name in ('support-j0','support-j1'):
        gate=w.read_json(root/name/'marginal-support-summary.json')
    ...
    assert not (root/'.h4l-mass-off-v3-claims').exists()
...
def test_history_cannot_be_cleared_by_new_output(tmp_path,monkeypatch):
    ...
    assert not (root/'new-v3-output').exists()
    ...
    receipt=w._seal({'schema_version':'h4l-off-assessment-access-v3','specification_id':spec['specification_id'],
    ...
    access=root/'v3-review.json'; access.write_text(json.dumps(receipt))
...
def test_script_matrix_and_immutable_report_resume(tmp_path,monkeypatch,capsys):
    ...
    spec=importlib.util.spec_from_file_location('evaluate_v3_test',script)
...
def test_failed_gate_continuation_reuses_only_bound_report(tmp_path,monkeypatch,capsys):
    ...
    spec=importlib.util.spec_from_file_location('off_v3_gate_test',script)
    module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
    monkeypatch.setattr(module,'RUNS_ROOT',tmp_path)
    ...
    (root/'support-j0'/'marginal-support-summary.json').write_text(json.dumps(gate))
