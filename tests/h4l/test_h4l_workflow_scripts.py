...
def test_prepare_plan_stops_after_audit_and_prepare(
    ...
    assert "higgsml.cli templates" not in output
...
def test_g1_plan_reuses_prepared_run_without_preparing_root_again() -> None:
    ...
    assert completed.returncode == 0, completed.stderr
    output = completed.stdout
    assert output.count("higgsml.cli train") == 3
    assert output.count("higgsml.cli calibrate") == 5
    assert output.count("higgsml.cli templates") == 1
    assert "higgsml.cli prepare" not in output
...
def test_run_refuses_pending_t1_before_starting_batch(tmp_path: Path) -> None:
    ...
    assert "higgsml.cli train" not in completed.stdout
    assert not output_root.exists()
