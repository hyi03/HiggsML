...
from higgsml.cli import build_parser
...
from higgsml.workflow import execute, _claim_assessment, _require_unopened, _p0_audit, _require_g0, _population_id
from higgsml.modeling.discriminants import train_discriminant
...
def test_claim_precedes_payload_decode_and_failed_opening_blocks_training(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr('higgsml.workflow.load_research_data',decode)
...
def test_absolute_bridge_requires_matching_passed_g1_before_payload(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('higgsml.workflow.load_research_data',payload)
...
def test_report_binds_paired_curves_and_does_not_substitute_another_seed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('higgsml.workflow.write_learning_curves',plot)
