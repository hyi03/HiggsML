...
from higgsml.dataset_binding import dataset_context  # noqa: E402
...
def _invoke(arguments: list[str], *, plan_only: bool) -> None:
    ...
    if return_code != 0:
        raise WorkflowError(
            f"higgsml failed with exit code {return_code}",
            return_code,
        )
...
def _validate_run_root(run_root: Path, *, continue_run: bool) -> None:
    try:
        ...
        raise WorkflowError(
            f"RunRoot must be a child of the runs directory: {run_root}", 4
        ) from error
