...
Run from the repository root: python -m scripts.research_performance_benchmark --output PATH
...
CURRENT_MODULES = {
    'calibration': 'higgsml.modeling.calibration',
    'templates': 'higgsml.inference.templates',
    'inference': 'higgsml.inference.likelihood',
    'discriminants': 'higgsml.modeling.discriminants',
}
...
def baseline_module(name, revision, directory):
    ...
    spec = importlib.util.spec_from_file_location(f'higgsml._baseline_{name}',path)
...
def root_probe(directory,repeats):
    ...
    from higgsml.data import iter_development_events
...
def main(argv=None):
    ...
    from higgsml.data import development_spans
    ...
    with tempfile.TemporaryDirectory(prefix='higgsml-benchmark-') as directory:
        for name in ('calibration','templates','inference','discriminants'):
            ...
            new=importlib.import_module(CURRENT_MODULES[name])
            if name=='calibration':
        ...
        new=importlib.import_module('higgsml.inference.likelihood')
        import pyhf
    ...
    report=dict(baseline=args.baseline,platform=platform.platform(),python=platform.python_version(),
        implementation_sha256={str(path.relative_to('src')):hashlib.sha256(path.read_bytes()).hexdigest()
                               for path in sorted(Path('src/higgsml').rglob('*.py'))},
