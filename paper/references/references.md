...
| §3、§6：raw 与去相关方法的边界（扩展背景） | R06–R12、R27 | 当前结果未执行 CDF／对抗对照 |
| §1、§3、§5：表示与 AUC／推断目标 | R13、R14、R28 | 80 个 off 身份、单质量箱、配对 W68；容量对照未完成 |
| 似然与评价章节：Asimov、有限 MC、区间覆盖 | R15–R21 | 带符号组方差、shapesys 适用性、Toy 覆盖 |
| §4–§6：特征归因与探索选择 | R22–R26 | 常数 M0off、全子集、种子枚举；学习曲线与独立确认未完成 |
...
v0.3 正文已改用稳定 R 键并链接本文件。下表仅保留 v0.2 的历史数字编号，不能作为当前章节或待填结果状态。各条目中旧 § 号为首次整理时的拟引用位置，当前使用位置以上方映射及正文为准。
...
|---|---|---|
| 首次将 ML 用于四轻子或首次使用矩阵元 | 在无显式 m4l 输入和实际单质量箱条件下，比较完整 off 子集的 μ 区间宽度 | R01–R03、R14 |
| 首次提出质量去相关／条件可逆变换 | 当前只报告 raw 阈值和共同模板；CDF 为独立扩展，未提供本次结果 | R06–R12、R20、R21、R27 |
| Shapley 揭示了唯一物理信息或因果贡献 | 对已分别训练并冻结的子集及共同推断流程作区间宽度归因 | R22–R24 |
...
3. **signed-CDF 估计器（仅保留为扩展时）。** R12 仅是概率变换背景；需给出当前非负约束估计的目标函数、总量约束、误差处理、偏差及覆盖。不能暗示上述文献已验证此具体组合。
...
## 2026-09-21 SciSpace 更新与本稿引用决策

本次两次自然语言查询及返回记录见[检索日志](literature-search-log.md#2026-09-21-更新)。只读取题名、索引书目和摘要，未精读全文，也未重新核验所有旧 A 级条目；原 A 级标记保留其 2026-09-14 的历史核验含义。

- **R14：** 再次检得 INFERNO，用于说明分类目标与参数不确定度目标不同。SciSpace 将预印本索引年份显示为 2022，本稿沿用此前交叉核验的 2019 正式期刊版本，不修改为索引年。
- **R29：** 再次检得 arXiv:2401.10542，摘要讨论特定高统计计数实验、有限 MC 与 nuisance 下的欠覆盖。保留原有已核验出版信息，本次不以摘要为本项目 signed-MC／低支持情形的适用性证明。
- **R22–R25：** 沿用原有归因与选择文献。此次检得其他 Shapley 方法的价值函数和分布假设不同，不将它们的置信区间公式移植为本项目 W68 误差。
- **候选更新：** 2025 Higgs Signal Strength Estimation with Machine Learning under Systematic Uncertainties 与 2026 Learning to bin 可作未来推断目标／分箱工作的相关背景。本次未核验全文和正式版本，不加入正文性能比较，也不据此调整已打开 assessment 的网格。

本稿的实证支持来自[本地结果证据](../result-evidence.md)，不来自文献中的实验数字。没有依据本次定向检索提出“首次”或超越矩阵元、最优传输、INFERNO 等算法的主张。
