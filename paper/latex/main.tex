...
the registered T1 prescription. Each block contains sixteen candidates. In
the selected test01 aggregate snapshot, all fifteen model-self and all
fifteen assessment cells have valid numerical outputs. Table~\ref{tab:validation}
reports the available conditional coverage summaries. Numerical completion is
separate from physical applicability and independent validation; neither is
established by these internally generated diagnostics.
...
Table~\ref{tab:validation} records the current availability of conditional
coverage for the three highlighted candidates in the same selected snapshot
used by the Chinese manuscript. These numerical summaries require independent
coverage validation and do not establish that the narrowest interval is the
most reliable.

\begin{table*}[tbp]
\caption{Conditional coverage at $\mu=1$ and the status of the validation
layers. Numerical entries, when available, are five-seed medians of the
reported conditional coverage; a dash denotes an incomplete seed vector.
T2 averages inner conditional coverage within each outer calibration
procedure before the seed summary. These are not pooled confidence intervals
or independent validation results.}
...
Only simulated members of the cited collection were used. Both drafts bind
report \texttt{3fe3e15f}; its fourteen original aggregate sources and manifest
hashes were verified in the local test01 archive on 23 September 2026. The
evidence index records full identities, distinct execution revisions, and
reproduction instructions. The unfinished active run is separate. This
manuscript update retrained no models, refitted no likelihoods, and generated
no pseudo-experiments. A permanent archival release remains pending.
