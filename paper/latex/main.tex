...
the currently checked aggregate snapshot, the requested model-self and
assessment coverage cells are marked as incomplete seed vectors, so numerical
coverage summaries are not reported. This numerical incompleteness is separate
from the physical applicability and independence of those diagnostics.
...
coverage for the three highlighted candidates. A dash denotes an incomplete
five-seed vector in the checked aggregate snapshot. The missing summaries
reinforce that the interval construction requires further coverage study;
they do not support an unqualified statement that the narrowest interval is
the most reliable.
