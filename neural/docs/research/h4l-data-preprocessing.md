...
本文说明 `neural/src/research` 当前 H4l 研究流程在模型训练前如何读取、筛选、重建、变换和封存数据。本文只描述 MC-only educational/technical workflow，不构成 ATLAS 结果、Higgs discovery 或物理测量。

本文面向独立的 `higgsml-research` / `src.research` 路径，不描述历史 `higgsml-preprocess` 固定 15 特征分类器。两条路径复用部分 `src.domain` 选择与重建代码，但输入契约、持久化字段、角色划分和 `m4l` 使用规则不同。
...
ROOT 转换由 `src.research.data.export_research_data()` 实现；训练前加载由 `load_research_data()` 实现；模型侧标准化由 `src.research.discriminants.train_discriminant()` 实现。
...
- research protocol，例如 `config/research_protocol_v2.json`；
...
Research prepare 读取 `config/preprocess_protocol_mass_window.yaml` 中的基础选择，再用当前 research protocol 的 `mass_window` 覆盖其中的 `m4l_window_gev`。因此，具体 H4l research 质量支持域由 research protocol 决定，而不是固定采用历史预处理文件中的 `105–160 GeV`。
...
Z1 定义为最接近名义 Z 质量的 SFOS 对，Z2 由剩余轻子组成；配对和 tie-break 由 `src.domain.reconstruction` 冻结实现。
...
`m4l` 单独保存，不计入 engineered19。当前 `src.research` protocol 明确允许每种研究表示把 `m4l` 作为共同质量条件；这个例外不改变历史固定 15 特征分类器禁止使用 `m4l` 的规则。
...
特征顺序由 `src.research.representations` 固定，不由 DataFrame 当前列顺序决定。
...
- `src/research/data.py`：ROOT export、来源总体、角色、权重、G0、序列化和加载验证；
...
- `src/research/representations.py`：研究表示和有序特征集合；
- `src/research/discriminants.py`：train-only scaler、优化器权重和对抗质量分箱；
- `src/research/workflow.py`：prepare/train 阶段编排和门禁。

相关文档：

- [`H4l-Research-Project.md`](H4l-Research-Project.md)：研究目标、角色和科学解释边界；
- [`../sw-dev/h4l-research-runbook.md`](../sw-dev/h4l-research-runbook.md)：运行命令、阶段和 artifact 使用；
- [`../sw-dev/research-software-design.md`](../sw-dev/research-software-design.md)：软件组件与隔离设计；
- [`preprocess-protocol.md`](preprocess-protocol.md)：历史主预处理流程的科学语义，不等同于本文的 research export；
- [`../sw-dev/artifact-schema.md`](../sw-dev/artifact-schema.md)：通用 artifact 与完整性契约。
