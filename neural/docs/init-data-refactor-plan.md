...
4. 使用 `urllib.request` 直接 HTTP/HTTPS 请求，下载到本次调用独占临时文件。保留分块 I/O 与进度显示，设置有限连接/读取超时及有界重试；连接中断或响应截断后，在本次调用内使用 Range 请求续传，仅在 206 的 Content-Range 起点、终点及总大小合法时追加。服务器返回 200 时清空临时内容重下，完整文件哈希不符也清空后重试。最终仍校验固定大小和 SHA-256；调用结束清理本次临时文件，不支持跨运行续传。
