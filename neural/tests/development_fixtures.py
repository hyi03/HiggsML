...
def write_synthetic_preprocess_run(allowed_root: Path, *, poison_test_feature: bool = False,
                                   dataset: str = "atlas2020_4lep", inclusive: bool = False, frame_override=None) -> tuple[Path, pd.DataFrame]:
    ...
    if inclusive:
        import numpy as np
        frame = frame.drop(columns="train_weight")
        # Distinct masses avoid degeneracy and span far outside the old window.
        frame["m4l"] = 60.0 + np.arange(len(frame), dtype=float) * 4.0
    if frame_override is not None:
        frame = frame_override(frame)
    columns = tuple(frame.columns)
    context = dataset_context(dataset)
    protocol = load_preprocess_protocol(Path(__file__).parents[1]/("config/preprocess_protocol_inclusive.yaml" if inclusive else "config/preprocess_protocol_mass_window.yaml"), dataset=dataset)
    ...
    for partition,mask in (("development",frame.split!="test"),("test",frame.split=="test")):
        ...
        payload=canonical_csv_bytes(selected,columns,
    ...
    manifest={"schema_version":protocol.raw["schema_version"],"dataset_binding":context.snapshot(),"status":"success","run_type":"preprocess",
        "protocol_id":protocol.protocol_id,"started_at_utc":"fixture","completed_at_utc":"fixture",
        "inputs":list(context.samples.values()),"configuration":{"protocol_path":"config/preprocess_protocol_inclusive.yaml" if inclusive else "config/preprocess_protocol_mass_window.yaml",
        ...
        "schema":{"ordered_columns":list(columns),"dtypes":{n:str(frame[n].dtype) for n in columns}},
