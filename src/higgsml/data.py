...
def export_research_data(manifest_path, profile_path, protocol, *, max_entries=4096,
    ...
    """
    from higgsml.dataset_binding import dataset_context
    from higgsml.config import load_selection_protocol
    ...
    from higgsml.physics.weights import physical_event_weight
    import uproot
    ...
    selection_protocol = load_selection_protocol(
        Path(__file__).resolve().parents[2] / "config/protocols/h4l_selection_v1.yaml",
        dataset=p["dataset"],
    )
    selection = SelectionConfig.from_mapping(
        dict(selection_protocol.selection, m4l_window_gev=p["mass_window"])
    )
